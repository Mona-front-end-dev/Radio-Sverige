...
const Navbar = (props) => {
  ...
  const onCollapseMenu = () => {
    ...
    if (menuCollapsed) document.body.classList.remove("menu-open");
  ...
  return (
    <header className={styles.topHeader}>
      ...
      <nav className={`${styles.navbar} ${styles.md}`}>{renderLinks()}</nav>
      <nav
        className={`${styles.navmob} ${menuCollapsed ? styles.expanded : ""}`}
      >
        <FontAwesomeIcon
          icon={faTimes}
          size="2x"
